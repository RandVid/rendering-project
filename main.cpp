...
int main()
{
    ...
    // ---------------- TELEPORT CHAIN ----------------
    // Use Left/Right arrows to select a node, Up arrow to teleport to it.
    // (Does not replace WASD movement; it just snaps camera to preset viewpoints.)
    TeleportNode* tp0 = makeNode("Start", Vector3(0, 0, 10), (Z * -1 + X * 0.001));
    TeleportNode* tp1 = makeNode("Above red sphere", Vector3(0, 10, 12), Vector3(0, 0, -1));
    TeleportNode* tp2 = makeNode("Look at box", Vector3(0, 6, 6), Vector3(0, 1, -0.2));
    TeleportNode* tp3 = makeNode("Side view", Vector3(15, 10, 6), Vector3(-1, 0, -0.1));
    linkNodes(tp0, tp1);
    linkNodes(tp1, tp2);
    linkNodes(tp2, tp3);
